...
class Client {
    ...
    if (this.scopes.length > 0 && this.redirectURI !== '') {
      ...
      return {
        link: `https://discord.com/api/oauth2/authorize?response_type=code&client_id=${this._id}&scope=${this.scopes.join(
          '%20'
        )}&state=${state}&redirect_uri=${this.redirectURI}`,
        state
      };
   ...
   * @returns {Promise<object>} tokenResponse The access token response returned by the discord for the user.
   *
   * @example let tokenResponse = myClient.raw(userKey);
   */
  async raw(key) {
    return new Promise((resolve, reject) => {
      jwt.verify(key, this._secret, (err, access) => {
        if (err) reject(new Error('Invalid key provided'));
        else resolve(access);
      });
    });
  }
   ...
   */
  async checkValidity(key) {
    const access = await this.raw(key);
    return {
